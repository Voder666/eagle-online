...
def update_flask_url():
    try:
        ...
        # 假设url.json位于脚本目录的相对路径 build 子目录中
        # 注意：请根据实际目录结构调整 'build' 子目录的位置
        url_json_path = os.path.join(script_dir, 'build', 'url.json')

        # 更新 url.json 文件
        with open(url_json_path, 'w', encoding='utf-8') as url_file:
            # 将新的 URL 写入 url.json
            json.dump({'backendURL': new_flask_url}, url_file, indent=4)

        # 返回成功消息
        return jsonify({"message": "flask_url and url.json updated successfully"}), 200
